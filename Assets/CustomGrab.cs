...
public class CustomGrab : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        doubleRotating = doubleRotation.action.IsPressed();
        if (grabbing)
        {
            ...
            if (grabbedObject)
            {
                ...
                Vector3 vectorToGrabbedObject = grabbedObject.position - transform.position;
                Vector3 rotatedVector = deltaRotation * vectorToGrabbedObject;
                Vector3 deltaRotatedPosition = rotatedVector - vectorToGrabbedObject;
                Vector3 deltaBasePosition = transform.position - lastHandPosition;

                grabbedObject.position = grabbedObject.position + deltaBasePosition + deltaRotatedPosition;
            }
        }
